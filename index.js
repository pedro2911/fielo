    ...
    if(typeof coordinates !== 'Object') {
        if(coordinates.i>=0 && coordinates.j>=0) {
            ...
            if(coordinates.j+1 < arrayInput.length) {
                res =  savePaths({i:coordinates.i, j: coordinates.j+1}, coordinatesStr+'->('+(coordinates.i)+','+(coordinates.j+1)+')',arrayInput[coordinates.i][coordinates.j+1]+weight);
            ...
            if(coordinates.i-1 >= 0 && coordinates.j+1 <  arrayInput[coordinates.j+1].length) {
                res =savePaths({i:coordinates.i-1, j: coordinates.j+1}, coordinatesStr+'->('+(coordinates.i-1)+','+(coordinates.j+1)+')',arrayInput[coordinates.i-1][coordinates.j+1]+weight);
                if(res.stop == 1 && largestPathsValue <= res.bananas) {
                    largestPathsValue = res.bananas;
                }
